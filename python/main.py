...
class DeviceTree:
	...
	def CreateGui(self):
		fhandle=open('/home/fog/python/gui.dat','w');
		fhandle.close();
	def CreateDtsFile(self):
		fd=open('/home/fog/python/DeviceTree.dts','w');
	...
	def ExecDownload(self):
		...
		while 1:
			if event1.wait(0.2):
				event1.clear()
				CommandStr='echo "123456"|sudo -S adb reboot-bootloader'
				(status,output)=commands.getstatusoutput(CommandStr) 
				UserStr=output
				CommandStr='echo "123456"|sudo -S fastboot '+FlashCommand+' '+PartitionName+' '+FileName
				(status,output)=commands.getstatusoutput(CommandStr) 
				UserStr=UserStr+output
				if 'flash'==FlashCommand:
					CommandStr='echo "123456"|sudo -S fastboot reboot'
					(status,output)=commands.getstatusoutput(CommandStr) 
					UserStr=UserStr+output
					pass
	...
	def on_filechooser_file_set(self, widget, data=None):
		...
		if os.path.isfile('/home/fog/python/Config.ini'):
			fd=open('/home/fog/python/Config.ini','w')
			fd.write(FileName)
			fd.close()
	...
	def destroy(self, widget, data=None): 
			global even1,event2 
			if os.path.isfile('/home/fog/python/gui.dat'):
				os.remove('/home/fog/python/gui.dat')
			event2.set()
			gtk.main_quit()
	...
	def CreateTree(self):
			...
			self.CreateGui();
			if os.path.isfile('/home/fog/python/Config.ini'):
				fd=open('/home/fog/python/Config.ini','r')
				FileName=fd.read()
				fd.close()
			else:
			...
			builder.add_from_file('/home/fog/python/gui.glade')
